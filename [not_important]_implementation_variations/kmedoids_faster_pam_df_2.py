...
# From initial assignment we have the first and second closed medoids for N embeddings 
#  i first second 
#  i j dist

def assignment_add(first_and_second_min , added_medoid , pair_wise_distance_df):
    pair_wise_distance_added_medoid = pair_wise_distance_df.filter(col("j") == added_medoid)
    first_and_second_min = first_and_second_min.alias("all").join(pair_wise_distance_added_medoid.alias("one") , col("all.i") == col("one.i"))
    first_and_second_min = first_and_second_min.withColumn('first_new',
                                                           when( col("dist") < col("first.dist") , struct("dist" , "j")
                                                                ).otherwise(col("first")))
    first_and_second_min = first_and_second_min.withColumn('second_new',
                                                           when( col("dist") == col("first.dist"), col("first")).when(
                                                               (col("dist") >  col("first_new.dist")) & (col("dist") < col("second.dist")), struct("dist" , "j")
                                                                ).otherwise(col('second')))
    first_and_second_min = first_and_second_min.drop('first').drop("second").withColumnRenamed("first_new" , "first").withColumnRenamed("second_new" , "second")
    first_and_second_min = first_and_second_min.withColumn("removal_loss", col("second.dist") - col("first.dist"))
...
if __name__ == "__main__":
    ...
    pair_wise_distance_df = pair_wise_distance_df.repartition(col("i"))
    ...
    for _ in range(maxIterations):
        ...
        for embedding in embeddings_ids_list:
            ...
            # first_and_second_min , added_medoid , pair_wise_distance_df
            all_medoids_assignments_df = assignment_add(assignment_df , embedding , pair_wise_distance_df)
