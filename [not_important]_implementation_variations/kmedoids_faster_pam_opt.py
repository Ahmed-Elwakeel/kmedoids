...
def initial_assignment(pair_wise_distance_df_filtered, k_medoids_list):
...
if __name__ == "__main__":
    ...
    pair_wise_distance_df_filtered = pair_wise_distance_df.filter(col("j").isin(k_medoids_list))
    pair_wise_distance_df_filtered.cache()
    assignment_df = initial_assignment(pair_wise_distance_df_filtered, k_medoids_list)
    ...
    for _ in range(maxIterations):
        ...
        for embedding in embeddings_ids_list:
            ...
            one_medoid_pair_wise_distance = pair_wise_distance_df.filter(col('j') == embedding)
            pair_wise_distance_df_filtered = pair_wise_distance_df_filtered.union(one_medoid_pair_wise_distance)

            # sc.broadcast(to_be_passed_list)
            all_medoids_assignments_df = initial_assignment(pair_wise_distance_df_filtered , to_be_passed_list)
            ...
            pair_wise_distance_df_filtered = pair_wise_distance_df_filtered.filter(col('j') != embedding)

            to_be_passed_list = list(k_medoids_set)
            # sc.broadcast(to_be_passed_list)
            best_so_far_medoids_assignments_df = initial_assignment(pair_wise_distance_df_filtered, to_be_passed_list)
